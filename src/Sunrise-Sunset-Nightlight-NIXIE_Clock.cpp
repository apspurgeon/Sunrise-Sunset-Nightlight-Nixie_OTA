...
//LED details
//#define NUM_LEDS_PER_STRIP 20 //Number of LEDs per strip
#define NUM_LEDS_PER_STRIP 82 //Number of LEDs per strip
...
const int LEDSecondstowait = 1; //Wait between LED updates (sec)
...
//Nixie vars
//char nixietable[] = {10,20,5,15,9,19,4,14,8,18,3,13,7,17,2,12,6,16,1,11};
char nixietable[] = {4, 14, 5, 15, 3, 13, 6, 16, 2, 12, 7, 17, 1, 11, 8, 18, 0, 10, 9, 19, 24, 34, 25, 35, 23, 33, 26, 36, 22, 32, 27, 37, 21, 31, 28, 38, 20, 30, 29, 39};
char nixietable2[] = {44, 54, 45, 55, 43, 53, 46, 56, 42, 52, 47, 57, 41, 51, 48, 58, 40, 50, 49, 59, 64, 74, 65, 75, 63, 73, 66, 76, 62, 72, 67, 77, 61, 71, 68, 78, 60, 70, 69, 79};
int nixienext = 20;
int divider = 1;
...
void DoTheLEDs()
{
  ...
  MSD_hour = int(local_hour / 10);
  LSD_hour = local_hour - (MSD_hour * 10);

  MSD_minute = int(minute / 10);
  LSD_minute = minute - (MSD_minute * 10);

  Serial.print("Hour MSD: ");
  Serial.print(MSD_hour);
  Serial.print(", Hour LSD: ");
  Serial.println(LSD_hour);

  Serial.print("Minute MSD: ");
  Serial.print(MSD_minute);
  Serial.print(", Minute LSD: ");
  Serial.println(LSD_minute);

  //1st & 2nd LED  (Hour)
  int LED1 = nixietable[MSD_hour * 2];
  int LED2 = nixietable[(MSD_hour * 2) + 1];
  int LED3 = nixietable[(LSD_hour * 2) + nixienext];
  int LED4 = nixietable[(LSD_hour * 2) + nixienext + 1];
  ...
  int LED5 = nixietable2[MSD_minute * 2];
  int LED6 = nixietable2[(MSD_minute * 2) + 1];
  int LED7 = nixietable2[(LSD_minute * 2) + nixienext];
  int LED8 = nixietable2[(LSD_minute * 2) + nixienext + 1];
  ...
  leds[LED1] = CRGB(red, green, blue);
  leds[LED2] = CRGB(red, green, blue);
  leds[LED3] = CRGB(red, green, blue);
  leds[LED4] = CRGB(red, green, blue);

  leds[LED5] = CRGB(red, green, blue);
  leds[LED6] = CRGB(red, green, blue);
  leds[LED7] = CRGB(red, green, blue);
  leds[LED8] = CRGB(red, green, blue);

  //Set the top light:  0=Same as other LEDs, 1=Red, 2=Green, 3=Blue, 4=White
  if (TARDIS == 0)
  {
    leds[NUM_LEDS_PER_STRIP - 1] = CRGB(red * divider, blue * divider, green * divider);
    leds[NUM_LEDS_PER_STRIP - 2] = CRGB(red * divider, blue * divider, green * divider);
  }

  //If in flash phase don't update the LEDs, let the Flash routine do this
  if (flash_phase == 0)
  {
    if (TARDIS == 1)
    {
      leds[NUM_LEDS_PER_STRIP - 1] = CRGB(255 * divider, 0, 0);
      leds[NUM_LEDS_PER_STRIP - 2] = CRGB(255 * divider, 0, 0);
    }

    if (TARDIS == 2)
    {
      leds[NUM_LEDS_PER_STRIP - 1] = CRGB(0, 255 * divider, 0);
      leds[NUM_LEDS_PER_STRIP - 2] = CRGB(0, 255 * divider, 0);
    }

    if (TARDIS == 3)
    {
      leds[NUM_LEDS_PER_STRIP - 1] = CRGB(0, 0, 255 * divider);
      leds[NUM_LEDS_PER_STRIP - 2] = CRGB(0, 0, 255 * divider);
    }

    if (TARDIS == 4)
    {
      leds[NUM_LEDS_PER_STRIP - 1].setRGB(255 * divider, 255 * divider, 255 * divider); //Light on top of TARDIS White
      leds[NUM_LEDS_PER_STRIP - 2].setRGB(255 * divider, 255 * divider, 255 * divider); //Light on top of TARDIS White
    }
...
void checkflash()
{
  ...
  if (flash_phase == 1)
  {
    ...
    DoTheLEDs(); //Added to make sure clock updates during flash
...
void test_LEDs()
{
  ...
  fill_solid(leds, 20, CRGB(255, 0, 0));
  FastLED.setBrightness(howbright);
  FastLED.show();
  Serial.println("TEST:  Red");
  delay(1000);

  fill_solid(leds, 20, CRGB(0, 255, 0));
  FastLED.setBrightness(howbright);
  FastLED.show();
  Serial.println("TEST:  Green");
  delay(1000);

  fill_solid(leds, 20, CRGB(0, 0, 255));
  FastLED.setBrightness(howbright);
  FastLED.show();
  Serial.println("TEST:  Blue");
  delay(1000);

  fill_solid(leds, 20, CRGB(0, 0, 0));
  FastLED.setBrightness(howbright);
  FastLED.show();
}
